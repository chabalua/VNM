...
function gotoTab(t) {
  ...
  if (t === 'order' && isDesktopLayout() && window.buildDesktopOrderSidebarHTML) {
    var _oside = document.getElementById('order-desktop-side');
    if (_oside) _oside.innerHTML = window.buildDesktopOrderSidebarHTML();
  }
  if (t === 'don') renderDon();
...
function renderHomeDashboard() {
  ...
  html += '<button class="action-card" onclick="gotoTab(\'order\')"><span class="action-icon">' + _ic('cart', 20, 1.8) + '</span><span>Đặt hàng</span></button>';
  html += '<button class="action-card" onclick="gotoTab(\'kh\')"><span class="action-icon">' + _ic('users', 20, 1.8) + '</span><span>Khách hàng</span></button>';
  html += '<button class="action-card" onclick="gotoTab(\'don\')"><span class="action-icon">' + _ic('list', 20, 1.8) + '</span><span>Đơn hàng</span></button>';
  html += '<button class="action-card" onclick="openKpiSettings(\'' + data.monthKey + '\')"><span class="action-icon">' + _ic('zap', 20, 1.8) + '</span><span>Mục tiêu KPI</span></button>';
  html += '<button class="action-card" onclick="syncOpenSettings()"><span class="action-icon">' + _ic('cloud', 20, 1.8) + '</span><span>Cloud</span></button>';
  html += '</div></div>';
...
var _layoutUpdateTimer = null;
function updateLayoutMode() {
  var nextMode = window.innerWidth >= 1024 ? 'desktop' : 'mobile';
  var current = document.documentElement.getAttribute('data-layout') || '';
  if (current === nextMode) return;
  document.documentElement.setAttribute('data-layout', nextMode);
  if (nextMode === 'desktop' && window.renderDesktopSidebar) window.renderDesktopSidebar();
  if (nextMode === 'desktop' && window.buildDesktopOrderSidebarHTML) {
    var side = document.getElementById('order-desktop-side');
    if (side) side.innerHTML = window.buildDesktopOrderSidebarHTML();
  }
}
...
window.addEventListener('resize', function() {
  clearTimeout(_layoutUpdateTimer);
  _layoutUpdateTimer = setTimeout(updateLayoutMode, 150);
});
