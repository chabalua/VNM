...
function kmTypeFields(prog) {
  ...
  if (t === 'order_bonus') {
    ...
    var cascadeToggle = '<div style="margin-bottom:8px;padding:8px;background:var(--bg2,#f5f5f5);border-radius:var(--Rs);"><label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px"><input type="checkbox" id="kf-bonus-cascade"' + (isCascade ? ' checked' : '') + ' onchange="document.getElementById(\'kf-fields\').innerHTML=kmTypeFields(kmReadForm());kmPreview()"><b>Cascade</b>: áp tier lớn trước, dùng phần dư cho tier nhỏ hơn</label><div style="font-size:10px;color:var(--t3);margin-top:3px">VD: đơn 9M → 1×5M (8 hộp) + 2×2M (4 hộp) = 12 hộp</div></div>';
